...
- **Nearly parallel edges are mollified.** For parallel edges the closest points are not unique, so the edge–edge distance has a gradient that jumps as the edges slide, and Newton oscillates. Following IPC, each edge–edge barrier is multiplied by `m(c) = (2 − c/ε_×) c/ε_×` for `c < ε_×`, else 1, where `c = ‖e_a × e_b‖²` and `ε_× = 10⁻³ ‖ē_a‖²‖ē_b‖²` (rest lengths). The weight is C¹ and vanishes for parallel edges. Non-penetration is still guaranteed there by the point–triangle pairs of the same primitives. Found when a block landing flat on the ground oscillated in Newton without converging.
