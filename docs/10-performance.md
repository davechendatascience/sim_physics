...
| Newton stall under a short pad (~1.2 mm travel) | every step at the 120 cap; pads lag their path, force → 900 N | diagnosed as contact lock, not snap-through (energy convex along the step; 10 sub-steps converge): a sliding pair sat at 0.054 µm and the sampled CCD hit its 1 024-sample cap. Barrier stiffness raised 10³ → 10⁵ (gaps stay a fair fraction of d̂) and the cap raised to 65 536 with chunking | 5–19 iterations per step; forces unchanged to 1.8% |
